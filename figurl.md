...
Data visualization is an important tool for scientists to effectively communicate their results and to collaborate with other researchers. However, traditional data visualization software can be limiting for collaborative research, as it can be difficult to share visualizations in an interactive format with other researchers or the broader scientific community. To address these challenges, we have developed Figurl, a web-based data visualization system that makes it easy to generate, view, share, and store web-based data visualizations from the convenience of Python scripts with minimal configuration. Figurl utilizes shareable links, content-addressable storage, and domain-specific visualization plugins to allow scientists to easily view and share their data in the browser. We demonstrate the utility of Figurl in two neuroscience contexts, illustrating the potential of this technology to facilitate data visualization and collaborative data sharing in the scientific community.

## Introduction

The clickable hyperlink is perhaps the most portable, versatile, and robust method for sharing data. Because web URLs are relatively short strings of text, they can be emailed, texted, uploaded to a database, or posted just about anywhere. As compared to working with files, URL links take up far less disk space, do not require special software for reading, and can be opened/launched from wherever they are found. Provided that the underlying data is stored in a reliable archive such as Zenodo [@zeonodo] URL permalinks can even be embedded in publications or long-term archives.
